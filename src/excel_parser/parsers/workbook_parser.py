...
# Fast-path backends. We prefer the Rust core (excel_core) because it emits
...
from excel_parser.parsers import calamine_core as _calamine_core
...
from excel_parser.models.common import CalculationMode, DateSystem, ParseError, Severity
from excel_parser.models.workbook import (
    ExternalLink,
    NamedRangeDTO,
    WorkbookDTO,
    WorkbookProperties,
...
class WorkbookParser:
    ...
    def parse(self) -> WorkbookDTO:
        ...
        # Compute file hash from the *original* source bytes so a workbook's
        # identity is tied to the file the user supplied (the .xls), not the
        # converted .xlsx produced below.
        ...
        # Legacy .xls (BIFF) cannot be read by openpyxl. Convert it to .xlsx
        # in-memory once, then point every downstream load at those bytes. On
        # failure the original error is recorded and an empty result returned.
        if not self._convert_legacy_xls_if_needed(result):
            return result
        ...
        # cascades to the next: (1) Rust excel_core (values + formulas),
        ...
        if self._build_dep_graph:
            try:
                from excel_parser.formula.dependency_builder import DependencyBuilder
    ...
    def _convert_legacy_xls_if_needed(self, result: WorkbookDTO) -> bool:
        """Convert a legacy ``.xls`` source to ``.xlsx`` bytes in place.

        When the input is a BIFF ``.xls``, this replaces ``self._content`` with
        the converted ``.xlsx`` bytes and clears ``self._path`` so every
        downstream load (openpyxl, the Rust/calamine fast paths, chart
        extraction) operates on the converted workbook. The display
        ``filename`` is left untouched.

        Returns:
            ``True`` if parsing should continue (no conversion needed, or
            conversion succeeded). ``False`` if conversion failed — an ERROR is
            appended to ``result`` and the caller should return early.
        """
        if not is_legacy_xls(
            filename=self._filename, path=self._path, content=self._content
        ):
            return True

        try:
            xlsx_bytes, warnings = convert_xls_to_xlsx(
                path=self._path, content=self._content
            )
        except XlsConversionError as exc:
            logger.error("Legacy .xls conversion failed: %s", exc)
            result.errors.append(ParseError(
                severity=Severity.ERROR,
                stage="load",
                message=f"Failed to convert legacy .xls workbook: {exc}",
            ))
            return False

        self._content = xlsx_bytes
        self._path = None
        for msg in warnings:
            result.errors.append(ParseError(
                severity=Severity.WARNING,
