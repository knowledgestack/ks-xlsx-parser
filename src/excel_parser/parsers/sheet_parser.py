...
from excel_parser.models.common import CellCoord, CellRange, ParseError, Severity, col_letter_to_number
...
from excel_parser.models.sheet import (
...
class SheetParser:
    ...
    def _extract_dimensions(self, sheet: SheetDTO) -> None:
        """Extract custom row heights, column widths, and outline levels."""
        for row_idx, rd in self._ws.row_dimensions.items():
            ...
            # Excel row grouping/outline level — a deterministic, nesting-aware
            # section signal (Data → Group). Only store non-zero levels.
            level = getattr(rd, "outline_level", 0) or getattr(rd, "outlineLevel", 0)
            if level:
                sheet.row_outline_levels[row_idx] = int(level)
