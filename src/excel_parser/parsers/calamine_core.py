...
This module is the *only* place in ``src/`` that imports ``excel_core``
...
See ``rust/excel_core/REMOVAL.md`` for the full removal procedure.
...
try:
    import excel_core  # type: ignore[import-not-found]
    _HAS_CORE = True
except ImportError:  # pragma: no cover - crate is an optional build artefact
    excel_core = None  # type: ignore[assignment]
    _HAS_CORE = False
...
def read_cells(
    ...
    try:
        if path is not None:
            ...
            # excel_core's read_workbook takes a path today. Writing to
        ...
        raw = excel_core.read_workbook(target)
    except Exception as exc:  # noqa: BLE001
        logger.warning("excel_core read failed, falling back: %s", exc)
        return None
    finally:
