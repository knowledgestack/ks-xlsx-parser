...
class SheetDTO(StableModel):
    ...
    # Excel row outline / grouping levels (Data → Group). row_num → level (>=1).
    # Deterministic, nesting-aware section structure straight from the file;
    # rows with no grouping are absent (level 0). See analysis/section_detector.
    row_outline_levels: dict[int, int] = Field(default_factory=dict)
