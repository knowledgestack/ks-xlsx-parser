...
Or use the excel-parser-api script (runs on port 8080 by default):
    excel-parser-api
...
<div class="container">
  ...
  <div class="upload-area" id="dropzone">
    ...
    <strong>Drop .xlsx or .xls file here or click to browse</strong>
    <p>Supports .xlsx, .xlsm, and .xls files up to 50MB</p>
...
async def parse_excel(file: UploadFile = File(...)):
    ...
    if not file.filename or not file.filename.lower().endswith((".xlsx", ".xlsm", ".xls")):
        return JSONResponse(
            status_code=400,
            content={"detail": "Only .xlsx, .xlsm, and .xls files are supported"},
        )
...
def main() -> None:
    """Run the API server on port 8080. Use: excel-parser-api"""
    import uvicorn
