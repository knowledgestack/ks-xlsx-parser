...
1. **Delete this directory**: `rm -rf rust/excel_core`.
2. **Drop the maturin build from CI** (wherever it's wired — today it's
   invoked as `maturin develop` inside `rust/excel_core`).
...
4. **Remove the optional-dep entry** `excel-core` from `pyproject.toml`.
5. **Run** `pytest tests/` to confirm nothing regresses.

No other code in `src/` imports `excel_core` directly — that's by design.
Grep for `excel_core` before removing to confirm the only references are
